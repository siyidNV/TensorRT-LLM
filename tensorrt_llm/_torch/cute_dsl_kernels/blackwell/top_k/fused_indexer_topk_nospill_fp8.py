"""Fused DeepSeek V3.2 (FP8) decode indexer + exact top-K, sm_100 CuTe DSL.

FP8 twin of fused_indexer_topk_nospill.py: the K cache holds 32-token pages of 128-byte
e4m3 rows followed by one fp32 scale per token, the query is plain e4m3, the MMA is the
unscaled tcgen05 fp8 kernel and the epilogue multiplies the relu-weighted head sum by the
token scale read straight from its page. Selection (16-bit keys, histogram descent, safe
line, row split, fp32 boundary refinement) is identical to the FP4 kernel.
"""
...
DIMB = 128  # bytes per token row (fp8)
PAGE = 32  # tokens per KV page
PGB = 4224  # bytes per page: 32 x 128 B fp8 rows, then 32 fp32 scales
SCALE_OFF = 4096  # byte offset of the scale plane inside a page
NBINS = 2048  # coarse bins = key >> 5 (signed-monotone 16-bit keys)
NFINE = 32  # fine bins  = key & 31
CAP = 2048
CROW = 16  # query rows per gmem copy sub-tile (128 threads x 16 B)
...
def _fp8_kernel(
    ...
    g2s = cute.make_copy_atom(
        cpasync.CopyG2SOp(cache_mode=cpasync.LoadCacheMode.GLOBAL), F8, num_bits_per_copy=128
    )
    qcp = cute.make_tiled_copy_tv(
        g2s, cute.make_ordered_layout((CROW, 8), order=(1, 0)), cute.make_layout((1, 16))
    )
    ...
    if tidx == 0:
        for i in cutlass.range_constexpr(STAGES):
            cute.arch.mbarrier_init(ab_full + i, 2)
    ...
    sA_ptr = cute.recast_ptr(sA_raw, sA_swz, dtype=F8)
    sB_ptr = cute.recast_ptr(sB_raw, sA_swz, dtype=F8)
    ...
    sK_tma = cute.make_tensor(cute.recast_ptr(sA_raw, dtype=F8), smem_layout_k_tma)
    ...
    acc_layout = cute.make_layout(LAY[2][0], stride=LAY[2][1])
    ...
    if tidx < 128:
        ...
        for r in cutlass.range_constexpr(HD * NEXT_N // CROW):
            src = cute.local_tile(gQ, (CROW, DIM), (r, 0))
            dst = cute.local_tile(sB_c, (CROW, DIM), (r, 0))
            cute.copy(qcp, thr_q.partition_S(src), thr_q.partition_D(dst))
        ...
        for j in cutlass.range(ntl, unroll=1):
            ...
            if cutlass.const_expr(_ASM):
                ...
                if jf < ntl:
                    ...
                    _pfl2(base + tkt * 256)
                    _pfl2(base + tkt * 256 + 128)
                    if tkt == 0:
                        _pfl2(base + SCALE_OFF)
            ...
            with cute.arch.elect_one():
                cute.arch.mbarrier_arrive_and_expect_tx(ab_full + s, 2 * PAGE * DIMB)
            cute.copy(
                tma_atom_k,
                gK_tma[(None, pg0)],
                sK_tma[(None, pgw, s)],
    ...
    # ---------------- consumer warps 0..11 ----------------
        ...
        for i in cutlass.range(gwg, ntl_pad, NGRP, unroll=1):
            ...
            if i < ntl:
                ...
                # the token's fp32 scale lives behind its page's data plane; tokens past the
                # row end (only on local tile 0) read page 0 so the address stays valid
                pgv = I32(0)
                if (i > 0) or (lane < lim0_[NEXT_N - 1]):
                    pgv = sBT[(i & (RBT - 1)) * 4 + (lane >> 5)]
                scv = cute.make_tensor(
                    cute.make_ptr(
                        F32,
                        kv_ptr.toint() + cutlass.Int64(pgv) * PGB + SCALE_OFF + (lane & 31) * 4,
                        GMEM,
                        assumed_align=4,
                    ),
                    cute.make_layout(1),
        ...
        if cutlass.const_expr(REFINE == 1):
            ...
            if warp_idx >= 12 and warp_idx < M_WARP:
                ...
                for j in cutlass.range(j0, JP + nvt, NGRP, unroll=1):
                    ...
                    q1 = cute.arch.add_packed_f32x2((acc[2][0], acc[2][1]), (acc[3][0], acc[3][1]))
                    q2 = cute.arch.add_packed_f32x2(q0, q1)
                    pgr = sPG[(j - JP) * 4 + lane // 32]
                    scr = cute.make_tensor(
                        cute.make_ptr(
                            F32,
                            kv_ptr.toint() + cutlass.Int64(pgr) * PGB + SCALE_OFF + (lane % 32) * 4,
                            GMEM,
                            assumed_align=4,
                        ),
                        cute.make_layout(1),
...
def _launch(
    ...
    sA_layout = sm100_utils.make_smem_layout_a(tiled_mma, mt, F8, STAGES)
    sB_layout = sm100_utils.make_smem_layout_b(tiled_mma, mt, F8, 1)

    # Present the paged allocation as (token-within-page, dim, physical-page); one fp8 per byte.
    mKV = cute.make_tensor(
        cute.recast_ptr(kv_ptr, dtype=F8),
        cute.make_layout(
            (PAGE, DIM, NPAGES),
            stride=(DIM, 1, PGB),
        ),
...
def run(
    ...
    """q_fp8 [B, next_n, 64, 128] u8 (e4m3 rows), weights [B * next_n, 64] f32,
    ...
    if fn is None:
        ...
        _cache[key] = fn
    fn(kv_ptr, q_ptr, w_ptr, clen_ptr, bt_ptr, oi_ptr, ov_ptr, ws_ptr, stream)
